...
Let $N$ be the total nodes.
Let $d_{min}$ be the minimum degree.
Let $d_{max}$ be the maximum degree (cutoff).
...
For many social networks, $\alpha \approx 2.5$.
The integral becomes $\int d^{-0.5} \, \mathrm{d}d = [2d^{0.5}]$.
...
In our model, the "Private" nodes are only those with degree $d < d_{cutoff}$ (where $d_{cutoff}$ is the degree of the smallest Public Hub).
The noise added by a private node $u$ is proportional to its *actual* degree $d_u$ (Smooth Sensitivity), not $d_{max}$.
So we integrate the *actual* squared degrees up to $d_{cutoff}$.
